...
export default function EditorHeader({
  ...
  const leftContent = (
    <div className="flex items-center">
      {/* Logo Placeholder */}
      <div className="w-10 h-10 bg-zinc-800/50 rounded-xl border border-zinc-700/50 flex items-center justify-center mr-8">
      ...
      <div className="flex items-center gap-3 text-xs md:text-sm text-zinc-400">
        {isEditing ? (
          <input
            ...
            className="bg-zinc-800 text-white px-2 py-0.5 rounded border border-zinc-700 focus:border-yellow-400 outline-none font-semibold max-w-[12rem] md:max-w-[18rem]"
          />
        ) : (
          <span 
            className="font-semibold text-white truncate max-w-[12rem] md:max-w-[18rem] cursor-pointer hover:bg-zinc-800 hover:text-yellow-400 px-2 py-0.5 rounded transition-colors"
        ...
        {selectedTool && (
          <>
            <div className="h-4 w-px bg-zinc-800 mx-2" />
            <span className="text-yellow-400 font-medium">{selectedTool}</span>
          </>
        )}
      </div>
    </div>
  ...
  const centerContent = (
    <div className="hidden md:flex items-center gap-1 bg-zinc-900/70 border border-zinc-800 rounded-full px-1 py-1 shadow-[0_18px_40px_rgba(0,0,0,0.55)]">
      <button
      ...
      <button
        onClick={onUndo}
        className="w-8 h-8 rounded-full border bg-zinc-900 text-zinc-300 border-zinc-700 hover:bg-zinc-800 hover:text-white flex items-center justify-center transition-all"
        title="Undo"
      >
  ...
  const rightContent = (
    <div className="flex items-center gap-2 md:gap-3">
      ...
      {userCredits !== undefined && (
        <Link href="/dashboard/credits" className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-zinc-900/50 border border-zinc-800 hover:border-zinc-700 transition-colors mr-1">
      ...
      <button
        ...
        className={`btn-compact rounded-full border text-xs md:text-sm px-4 ${
          outputPanelOpen
            ...
            : 'bg-transparent text-zinc-200 border-zinc-700 hover:bg-zinc-900'
        }`}
      ...
      <button
        type="button"
        className="hidden sm:inline-flex items-center justify-center rounded-full border border-cyan-500/40 bg-cyan-500/10 px-4 py-1.5 text-xs md:text-sm text-cyan-300 hover:bg-cyan-500/20 transition-colors"
      ...
      {CLERK_ENABLED ? (
        <div className="pl-2 ml-1 border-l border-zinc-800 flex items-center gap-2">
          <SignedOut>
            <SignInButton mode="modal">
              <button
                type="button"
                className="hidden md:inline-flex items-center justify-center rounded-full border border-zinc-700 px-3 py-1.5 text-[11px] text-zinc-300 hover:bg-zinc-900 transition-colors"
              >
                Sign in
              </button>
            </SignInButton>
          ...
          <SignedIn>
            <UserButton
              afterSignOutUrl="/"
              appearance={{ elements: { avatarBox: 'w-8 h-8 rounded-full border border-zinc-700 shadow-sm' } }}
            />
          </SignedIn>
        </div>
      ) : null}
